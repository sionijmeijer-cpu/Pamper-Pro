...
const ShoppingCart: React.FC<ShoppingCartProps> = ({ items = [], onCheckout }) => {
  ...
  const shippingCost = (user as any)?.subscription_tier === 'premium' ? 0 : 5;
        ...
        <CardContent className="space-y-3">
          ...
          <div className="flex justify-between">
            <span className="text-gray-600">
              Shipping {(user as any)?.subscription_tier === 'premium' ? '(Free)' : ''}
            </span>
        ...
        <Card className="bg-blue-50 border-blue-200">
          <CardContent className="pt-4">
            <p className="text-sm text-gray-700">
              💝 <strong>{((user as any).subscription_tier).toUpperCase()}</strong> member discount applied!
            </p>
          </CardContent>
        </Card>
      )}
